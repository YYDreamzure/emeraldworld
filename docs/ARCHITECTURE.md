...
## Local simulation (`sim/` + `client/`)

This repository includes a **runnable local stack** (Ollama, FastAPI, WebSocket, React Three Fiber) that implements the same design principles with JSONL persistence instead of PostgreSQL. See [LOCAL_SIMULATION.md](../LOCAL_SIMULATION.md) (complete guide) and [LOCAL_SIM.md](./LOCAL_SIM.md) (feature matrix).
---