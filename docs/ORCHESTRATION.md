...
Implemented in `sim/runner.py`, `sim/turn.py`, `sim/reactive.py`, `sim/needs.py`: round-robin, boost queue, reactive overhearing (2 tool calls), energy/knowledge/influence decay, live weather via Open-Meteo (Singapore by default), scaled time via `SIM_TIME_SCALE`. See [LOCAL_SIM.md](./LOCAL_SIM.md).
